#include "enc28j60.h"
// BASED on https://narodstream.ru/stm-urok-68-lan-enc28j60-chast-2/
//--------------------------------------------------
...
void enc28j60_ini(void)
{
	...
	LD_OFF;
	enc28j60_writeOp(ENC28J60_SOFT_RESET,ENC28J60_SOFT_RESET,0); //??? data not needed
	HAL_Delay(2);
	...
	//Enable promiscous mode!!! Enable Broadcast
...
uint16_t enc28j60_packetReceive(uint8_t *buf,uint16_t buflen)
{
	...
	if(enc28j60_readRegByte(EPKTCNT)>0)
	{
		...
		if(len>buflen)
			len=buflen;
		if((header.status&0x80)==0) //no valid CRC or symbol errors
			len=0;
		else
...
{
	long long i = 0;
	while((enc28j60_readOp(ENC28J60_READ_CTRL_REG,ECON1)&ECON1_TXRTS)  && (i++ < 10000)) //my modification, ! removed
	{
		if(enc28j60_readRegByte(EIR)&EIR_TXERIF)
		{
			...
			//https://forum.mysensors.org/topic/536/problems-with-enc28j60-losing-connection-freezing-using-uipethernet-or-ethershield-read-this/2
